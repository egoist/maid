    ...
    : v
        ...
        .replace(/(.+)(`(.+?)`)(.+)?$/, (s, $1, $2) => {
          let regexMatch = s
            .split(/(.+)(`(.+?)`)(.+)$/)
            .filter(g => g.trim() !== '')
          return `${$1}${$2} before this${
            regexMatch[regexMatch.length - 1] === s
              ? ''
  ...
  if (isParallelGroup(parallelOrWatch)) {
    ...
    watchTargets =
      ...
      regexGroups[regexGroups.length - 1].includes('`')
        ? regexGroups[regexGroups.length - 1]
            .replace(/`/g, '')
            .split(',')
            .map(target => target.trim())
        : null
    if (watchTargets !== null && watchTargets.length === 1)
      watchTargets = watchTargets[0]
  } else if (isWatchGroup(parallelOrWatch)) {
    watchTargets = regexGroups[regexGroups.length - 1]
      .replace(/`/g, '')
      .split(',')
      .map(target => target.trim())
    if (watchTargets.length === 1) watchTargets = watchTargets[0]
  }

  inParallel = /(.+?)in\s+parallel(.+?)?/i.test(v)
...
module.exports.fixCommand = fixCommand
