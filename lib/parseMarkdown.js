...
const isParallelGroup = regexGroupString =>
  /(.*?)in parallel/.test(regexGroupString)
...
const parseCommand = v => {
  ...
  const [, command, when, parallelOrWatch] = regexGroups
  ...
  if (isParallelGroup(parallelOrWatch)) {
    ...
    watchTargets =
      isWatchGroup(isCommandRe.exec(v)[isCommandRe.exec(v).length - 2]) &&
      isCommandRe.exec(v)[isCommandRe.exec(v).length - 1].includes('`')
        ? isCommandRe.exec(v)[isCommandRe.exec(v).length - 1]
        : null
  } else if (isWatchGroup(parallelOrWatch)) {
    watchTargets = isCommandRe.exec(v)[isCommandRe.exec(v).length - 1]
  }
...
const isMaidSectionComment = token => {
  return (
    ...
    /^<!--\s+maid-tasks\s+-->$/.test(token.content.trim())
  )
}
