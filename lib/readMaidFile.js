...
const parseBlockquote = (task, tokens) => {
  let endIndex
  ...
  const content = inlineTokens.map(token => token.content).join('\n')
  ...
  for (const [index, token] of tokens.entries()) {
    if (token.type === 'heading_open' && token.tag === 'h2') {
      ...
      for (const [index2, token2] of restTokens.entries()) {
        if (token2.type === 'heading_open' && token2.tag === 'h2') {
          break
        }
        if (token2.type === 'blockquote_open') {
          parseBlockquote(task, restTokens.slice(index2))
        }
